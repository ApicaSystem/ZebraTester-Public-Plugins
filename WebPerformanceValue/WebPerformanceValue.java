 ...
 */
public class WebPerformanceValue implements LoadtestPluginInterface
{
	private float wpmValue = -1.0f;		// input parameter #1 - label "wpmValue"
	
	private LogVector logVector = null;		// internal log vector - use logVector.log(<String>) to write log data
	private DataCollectionSequence simpleSequence = new DataCollectionSequence(1, "ApicaXValue", "", "Value", null);	
	 ...
	 *
	 * input parameter #1: (float) wpmValue / default value = '-1' / label "wpmValue"
	 *
	...
	public void setInputParameter(int parameterNumber, Object parameterValue)
	{
		...
		{
			case 0:
				wpmValue = Float.valueOf((String) parameterValue).floatValue();
				break;
			default:
				break;
		}
	...
	/**
	 * Execute plug-in at end of loop (per loop).
	 *
	...
	public void execute(Object context)
	{
		...
		CookieHandler cookieHandler = pluginContext.getCookieHandler();
			...
			DataCollectionFloatItem cf_item = new DataCollectionFloatItem(System.currentTimeMillis(), wpmValue);
		
			simpleSequence.addItem(cf_item);
		
			performanceData.addDataCollectionSequence(simpleSequence);
	...
	public static void main(String[] args)
	{
		try
		{
			...
			WebPerformanceValue plugin = new WebPerformanceValue();
